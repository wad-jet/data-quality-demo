...
- **Ledger producer'а:** flush после каждой строки — `kill -9` не теряет
  последнюю запись.
- **Ограниченный дренаж (добор очереди: воркер опустошает её перед остановкой):** при остановке очередь добирается до 30 с;
