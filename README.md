...
- **consumer** — цикл приёма лишь «тэпает» сообщения в очередь; проверки,
...
- **at-least-once / never-lose:** офсеты consumer'а коммитятся только за
...
- **Backpressure без дропов:** при полном буфере воркера цикл отправителя
  ждёт — сообщения не пропускаются.
- **Ledger producer'а:** flush после каждой строки — `kill -9` не теряет
  последнюю запись.
- **Ограниченный дренаж:** при остановке очередь добирается до 30 с;
