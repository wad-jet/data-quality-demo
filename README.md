...
- Форматы отчёта: text (stdout) и JSON (`-out`) — для специалистов и
  ...
  человекочитаемые отчёты на русском: вердикт, «что проверяли» (записи
  инспектора: заложенные дефекты / дополнительные записи / ложные
  срабатывания), метрики простыми словами,
  таблица дефектов по видам с легендой «как читать колонки»,
  DLQ (раскладка по видам), таймлайн с барами,
