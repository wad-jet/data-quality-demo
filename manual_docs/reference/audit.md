...
- **Markdown** — `-from … -format md` — человекочитаемый отчёт на русском:
  ...
  (раскладка записей инспектора: заложенные дефекты / дополнительные
  записи на те же дефекты / ложные срабатывания + «подтвердились
  записи: N из M — это и есть Precision»), метрики простыми словами
  (recall/precision с дробями и процентами), таблица дефектов по видам
  (колонки «Заложено», «Найдено из заложенных», «Recall», «Всего записей»,
  «Ложных», «Precision») с легендой «Как читать колонки», DLQ
  (dead-letter queue; «должно быть» с раскладкой по видам, offline + topic,
  если есть; «совпадает» / «РАСХОЖДЕНИЕ»), таймлайн с ASCII-барами,
  вводной про время события и пометками о разрывах,
  «как проверить отчёт за 10 секунд».
