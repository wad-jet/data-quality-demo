...
func TestIntegrationCommitProcessed(t *testing.T) {
    ...
    if err != nil { t.Fatalf("ledger: %v", err) }
    for i := 0; i < n; i++ {
        ev := gen.Next()
    ...
    if err != nil { t.Fatalf("offset fetch: %v", err) }
    var committed int64 = -1
    for _, t := range resp.Topics {
        if t.Topic != topic { continue }
        for _, tp := range t.Partitions {
            if tp.Partition == 0 && tp.ErrorCode == 0 { committed = tp.Offset }
        }
    }
